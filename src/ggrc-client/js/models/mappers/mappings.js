...
export default can.Construct.extend({
  ...
  getAvailableMappings(type) {
    let allowedToMap = this.getAllowedToMapModels(type);
    let related = this.getRelatedModels(type);
  ...
  */
  getRelatedModels(object) {
    return this._getModelsFromConfig(object, 'related');
  },
}, {
  /*
    On init:
    kick off the application of mixins to the mappings
  */
  init: function (opts) {
    ...
    $.extend(this, this.createMappings(opts));
  },
  // Recursively handle mixins -- this function should not be called directly.
  reifyMixins: function (definition, definitions) {
    let that = this;
    let finalDefinition = {};
    if (definition._mixins) {
      _.forEach(definition._mixins, function (name) {
        if (!definitions[name]) {
          ...
          let mixin = that.reifyMixins(definitions[name], definitions);
