...
export default class StateValidator {
   ...
   * }
   * // creates validator
   * const validator = new StateValidator(initState, initInjection);
   */
  constructor(initState = {}, initInjection = {}) {
    /**
   ...
   * validator.addValidateActions(action1, action2, action3);
   */
  addValidateActions(...actions) {
   ...
   * [injections]{@link Injection}.
   * It sets new state each time, when it was called.
   * @example <caption>Generating new state</caption>
   * let validationState1;
   * let validationState2;
   * validator.validate();
   * validationState1 = validator.validationState();
   * validator.validate();
   * validationState2 = validator.validationState();
   * validationState1 !== validationState2 // true
   */
  validate() {
    const validateActions = this._validationActions;
    const injected = this._injected;
    const oldState = this._validationState;
    ...
    this._validationState = {...oldState, ...newState};
  }
