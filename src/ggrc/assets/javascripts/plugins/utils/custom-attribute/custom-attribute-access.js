...
export default class CustomAttributeAccess {
   ...
   * If it is called without params then returns all custom attributes.
   * @param {(Options|number)=} args - Args may be:
   ...
   * @return {CustomAttributeObject|CustomAttributeObject[]|undefined} - Custom
  ...
  _buildDefaultValidations(caObject) {
    const validator = caObject.validator;
    validator.addValidateActions(hasEmptyMandatoryValue);
  }

  /**
   * Returns filtered custom attribute objects by certain type.
   * @param {Options} options - {@link Options}.
   * @param {CUSTOM_ATTRIBUTE_TYPE} [options.type=CUSTOM_ATTRIBUTE_TYPE.GLOBAL] -
   *  {@link CUSTOM_ATTRIBUTE_TYPE}.
   * @return {CustomAttributeObject[]} - Filtered custom attribute objects.
   */
   ...
   * @param {CustomAttributeChange} change - The custom attribute change.
   */
  _applyChangeToCaObject(caObject, change) {
    caObject.value = change.value;
  }
